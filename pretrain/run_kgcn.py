...
import torch
from transformers import Seq2SeqTrainingArguments, SchedulerType, IntervalStrategy, BartConfig
import argparse
import enums
...
def pre_train(args,
    ...
    config = BartConfig(vocab_size=len(dataset.code_tokenizer) + len(dataset.nl_tokenizer),
                        ...
                        num_labels=2)
    model = KGCNBartForConditionalGeneration(config=config, entity_weight=entity_embedding, relation_weight=relation_embedding)
    # log model statistic
