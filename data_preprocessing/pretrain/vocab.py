...
class Vocab(object):
    ...
    def encode_sequence(self, sequence: Union[str, List[str]], is_pre_tokenized=False, max_len=100):
        """
        ...
        ids_max_len = max_len - 2
        if len(ids) >= ids_max_len:
            ids = ids[:ids_max_len]
            ids = [self.get_sos_index()] + ids + [self.get_eos_index()]
            attention_mask = [1] * len(ids)
        else:
            n_pad = max_len - len(ids) -1
            attention_mask = [1] * (max_len-n_pad)
            attention_mask.extend([0] * n_pad)
            ids = [self.get_sos_index()] + ids + [self.get_eos_index()]
            ids.extend([self.get_pad_index()] * (n_pad-1))
        ...
        #     attention_mask = [1] * len(ids)
        #     attention_mask.extend([0] * n_pad)
        #     ids.extend([self.get_pad_index()] * n_pad)
